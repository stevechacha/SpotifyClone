...
class SearchArtistViewController: UIViewController, UITextFieldDelegate, UITableViewDelegate, UITableViewDataSource {
    ...
    override func viewDidLoad() {
        ...
        let searchQuery = "Ed Sheeran" // Replace with dynamic user input or a hardcoded query
...
//                                    print("- \(track.name) (Album: \(track.album?.name))")
//                                }
//
//                            case .failure(let error):
                    ...
                    case .success(let artistDetails):
                        ...
                        ArtistApiCaller.shared.getArtistAlbums(artistID: artistID) { albumsResult in
                            ...
                            case .success(let albums):
                                print("Albums by \(artistDetails.name):")
                                for album in albums.items {
                                    print("- \(album.name) (Released: \(String(describing: album.releaseDate)))")
                                }
                                ...
                                if let firstAlbum = albums.items.first {
                                    AlbumApiCaller.shared.getAlbumTracks(albumID: firstAlbum.id!) { tracksResult in
                                        ...
                                        case .success(let tracks):
                                            print("Tracks from album '\(firstAlbum.name)':")
                                            for track in tracks.items ?? [] {
                                                let duration = track.durationMs.map { $0 / 1000 } ?? 0
                                                print("- \(track.name) (Duration: \(duration) seconds)")
                                            }
                                        case .failure(let error):
                                            print("Failed to fetch tracks for album \(firstAlbum.name): \(error)")
                                        }
                                    }
                                }
                        ...
                        ArtistApiCaller.shared.getRecommendationsArtist(
                            ...
                            case .success(let recommendations):
                                print("Recommended Tracks for \(artistDetails.name):")
                                for track in recommendations.tracks {
                                    let artists = track.artists?.map { $0.name ?? "Unknown" }.joined(separator: ", ") ?? "Unknown"
